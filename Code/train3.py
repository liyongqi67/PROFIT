...
testFile="/home/share/liyongqi/data2/zhihuFenzhi2/zhihuFenzhi2QuestionAndTopicIdAndQuestionDescriptionVocabularyVal.txt"
...
with open("train3loss1", "a") as f:
    ...
    for epoch in range(10000000):
        num=len(data3)/batchSize/3
        num=int(num)
        for num in range(num):

            dagdag=np.array(DAG[num%topic_num])
            dagdag=dagdag.reshape([-1,6])
            maskmask=np.array(Mask[num%topic_num])
            maskmask=maskmask.reshape([-1,6])
            
            q,l,qd=getBatch(data3,batchSize,num)
            ...
            q=q.reshape([batchSize, question_length])

            qd=qd.reshape([batchSize, questionDescription_length])
            
            l=l.reshape([batchSize,topic_num])

            
            _,_,loss,hou= sess.run([train_op,update_op,model.loss,model.alpha],feed_dict = {model.X1_inputs: q, model.X2_inputs: qd, model.X3_inputs: dataTopic,model.y_inputs: l,model.dag:dagdag,
            	model.mask:maskmask,model.batch_size: batchSize, model.tst: False, model.keep_prob: 0.5})
            ...
            if num%100==1:
                ...
                f.write("epoch:"+str(epoch)+" num:"+str(num)+" loss:"+str(loss))
                f.write("\n")
            if num%5000==4999:
                    ...
                    print('f1',f1)
                    if((f1)>bestPerformance):
                    	saver = tf.train.Saver()
                        bestPerformance=f1
                        save_path = saver.save(sess, "./train3/model.ckpt")
